...
export const CartPage = () => {
    ...
    const getBalance = () => {
        ...
        Axios.get('http://localhost:3002/getBalance', { params: { user_id: user_id } }).then((res) => {
            if (res.status === 201) {
        ...
        <div>
            ...
            <Stack direction='row' sx={{ flexWrap: 'wrap' }} className='cartPage' justifyContent='center'>
                        ...
                        <TableBody>
                            {cartItems.map((item, index) => (
                                <TableRow
                                    key={item.id}
                                    ref={(el) => {
                                        (tableRowRef.current![index] as HTMLTableRowElement | null) = el;
                                    }}>
                ...
                <Stack direction='row' justifyContent='space-around' sx={{ position: 'absolute', bottom: '15px', width: '100vw' }}>
                    ...
                    <Button variant='contained' color='success' size='large' sx={{ backgroundColor: '#91BE77', color: 'white', fontSize: '20px', minWidth: '100px' }} href='/book'>
                        <Typography>Continue booking</Typography>
                    </Button>
                    <Button
                        variant='contained'
                        color='success'
